...
<section class="wrap section catalog-section"><div class="catalog-toolbar"><div class="filters" aria-label="Формат отдыха">{#each filters as [value,key]}<a class:active={style===value} href={link(destination,value)} aria-current={style===value?'true':undefined} data-sveltekit-noscroll><T k={key}/></a>{/each}</div><select class="country-select" value={destination} aria-label="Направление" onchange={e=>goto(link(e.currentTarget.value,style),{noScroll:true})}><option value="all"><T k="all"/></option>{#each tours as tour}<option value={tour.id}><T k={tour.id}/></option>{/each}</select></div>
