<script>
 ...
 import {language, localized, tourCopy} from './language.js';
...
<form class="trip-picker wrap" action="/tours/" method="GET" onsubmit={submit}>
 ...
 <label><span><T k="directionLabel"/></span><select name="destination" bind:value={destination}><option value="all"><T k="allDirections"/></option>{#each tours as tour}<option value={tour.id}>{tourCopy(tour,$language).name}</option>{/each}</select></label>
